...
| **Version**         | 1.0                                                                   |
| **Date**            | 2026-04-07                                                            |
| **Author**          | Shrinidhi Rao (Dell Technologies - Ansible PowerScale Team)           |
| **JIRA Story**      | ECS02C-841 - "As an Ansible Developer, I want to create powerscale_job_info module to list jobs with state/type filters" |
...
|---------|----------------------------------------------------------------------------------------------|--------------|----------|
| FR-001  | Filter jobs by state (`running`, `paused_user`, `paused_system`, `paused_policy`, `paused_priority`) | ECS02C-841   | P0       |
| FR-002  | Filter jobs by type (`FlexProtect`, `AutoBalance`, `TreeDelete`, `SmartPools`, etc.)         | ECS02C-841   | P0       |
| FR-003  | Return a consistent schema: `job_id`, `job_type`, `state`, `progress`, `impact`, `priority`, `start_time`, `last_update`, `end_time` | ECS02C-841   | P0       |
| FR-004  | Sort jobs based on any job field                                                             | ECS02C-841   | P1       |
| FR-005  | Control the number of results via a `limit` parameter                                        | ECS02C-841   | P1       |
| FR-006  | Retrieve a single job by its numeric ID                                                      | ECS02-77     | P0       |
| FR-007  | List recently completed jobs (succeeded, failed, cancelled)                                  | ECS02-77     | P1       |
| FR-008  | Retrieve the Job Engine summary/status                                                       | ECS02-77     | P2       |
...
These acceptance criteria are derived directly from the JIRA story ECS02C-841:
...
4. **Future Extensibility**: A separate `job_info` module pairs naturally with a future `job` management module (for pausing, resuming, cancelling jobs) under the same JIRA epic (ECS02-77). This follows the established collection pattern of `resource_info` (read) + `resource` (manage).
...
*Document generated for JIRA Epic ECS02-77 / Story ECS02C-841.*
