...
|--------|-----------------------------------------------------------------------------------------------|--------------|
| FR-01 | The module SHALL retrieve Job Engine reports from OneFS via the Job Reports API | Requirements |
| FR-02 | The module SHALL support filtering reports by `job_type` (e.g., `SmartPools`, `FlexProtect`) | Requirements |
| FR-03 | The module SHALL support filtering reports by `job_id` (numeric job instance identifier) | Requirements |
| FR-04 | The module SHALL support filtering reports by `event_key` (report key name) | Requirements |
...
| FR-09 | The module SHALL handle API pagination automatically, collecting all pages of results | Requirements |
| FR-10 | The module SHALL return parsed report objects suitable for Ansible output processing | Requirements |
| FR-11 | The module SHALL return all reports when no filter parameters are specified | Requirements |
...
3. **Consistent with story**: The story references "event_key" as the parameter name
