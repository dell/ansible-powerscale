...
class Ldap(object):
    ...
    def _merge_ldap_create_params(self, ldap_create_params, ldap_parameters):
        ...
        if not ldap_parameters:
            return
        # Keys where a truthy value means include
        ...
        # Keys where an explicit non-None value means include
        non_none_keys = ('group_base_dn', 'provider_domain', 'authentication')
    ...
    def _should_remove_ldap_key(self, key, input_ldap, array_ldap):
        """Determine if a key should be removed from input_ldap. Returns True if key should be removed.
        Raises fail_json for unsupported groupnet modification."""
        key_lower = key.lower()
        ...
        if key_lower in ("group_base_dn", "provider_domain", "authentication"):
            # None means omitted (preserve server value); explicit False is valid for authentication
            return input_ldap[key] is None or input_ldap[key] == array_ldap.get(key)
    ...
    def _filter_ldap_keys(self, input_ldap, array_ldap):
        ...
        for key in list(input_ldap):
            if self._should_remove_ldap_key(key, input_ldap, array_ldap):
                del input_ldap[key]
        return input_ldap
