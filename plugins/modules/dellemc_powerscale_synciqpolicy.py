...
DOCUMENTATION = r'''
---
module: dellemc_powerscale_synciqpolicy
...
class PowerScaleSynciqPolicy(object):
    ...
    def get_synciq_policy_details(self, policy_name, policy_id, job_params=None):
        ...
        try:
            ...
            policies_list = self.api_instance.list_sync_policies().policies
            for policy in policies_list:
                if policy.name == policy_name or policy.id == policy_id:
                    return policy, is_target_policy

            if job_params and job_params['action'] in \
                    ('allow_write', 'allow_write_revert'):
                is_target_policy = True
                target_policies_list = \
                    self.api_instance.get_target_policies().policies
                target_policy = [policy for policy in target_policies_list
                                 if policy.name == policy_name or
                                 policy.id == policy_id]
                if target_policy:
                    if len(target_policy) > 1:
                        ...
                        return target_policy[0], is_target_policy

            return None, is_target_policy
...
def is_policy_modify(policy_obj, policy_param):
    ...
    if 'source_network' in policy_param and \
            not set(policy_obj.source_network.to_dict().items()) ^ set(policy_param['source_network']):
        modify_policy_dict['source_network'] = policy_param['source_network']
...
def get_powerscale_synciqpolicy_parameters():
