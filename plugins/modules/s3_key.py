...
- name: Create S3 Key and overwrite existing - expire old key after 30 min
  powerscale_s3_key:
    onefs_host: "{{onefs_host}}"
    api_user: "{{api_user}}"
    api_password: "{{api_password}}"
...
class S3Key(object):
    ...
    def get_key_details(self):
        ...
        msg = f"Getting S3 Key details for user {user} and access zone:" \
              f" {access_zone}"
        LOG.info(msg)
        try:
            s3_key_obj = self.protocol_api.get_s3_key(
                s3_key_id=user, zone=access_zone)
            ...
            if str(e.status) == "404":
                log_msg = f"S3 Key status for user {user} in access zone" \
                          f" {access_zone} is {e.status}"
                LOG.info(log_msg)
                return None
            else:
                error_msg = utils.determine_error(error_obj=e)
                error_message = f"Failed to get details of S3 Key" \
    ...
    def create_key(self):
        ...
        try:
            ...
            if not self.module.check_mode:
                ...
                msg = f"Successfully created the S3 key with params: " \
                      f"{s3_key_params}"   
                LOG.info(msg)
            return key_details

        except Exception as e:
            error_msg = f"Create S3 Keys for user {user} and zone:" \
    ...
    def delete_key(self):
        ...
        try:
            ...
            LOG.info(msg)
            if not self.module.check_mode:
                self.protocol_api.delete_s3_key(
                    s3_key_id=user, zone=access_zone)
            ...
            error_msg = f"Delete S3 key for user {user} in access zone:" \
...
class S3KeyCreateHandler():
    def handle(self, key_object, key_params, key_exists, key_details):
        ...
        S3KeyDeleteHandler().handle(
            ...
            key_exists=key_exists, key_details=key_details)
...
class S3KeyHandler():
    def handle(self, key_object, key_params):
        ...
        key_exists = key_details is not None and key_details.get("access_id") is not None
