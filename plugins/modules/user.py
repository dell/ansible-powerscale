...
class User(object):
    ...
    def is_user_modified_sensitive(self, user_details):
        ...
        if (self.module.params['enabled'] is not None and
                self.module.params['enabled'] != user_details['enabled']):
            return True

        if (self.module.params['password_expires'] is not None and
                self.module.params['password_expires'] != user_details.get('password_expires')):
            return True

        if (self.module.params['expiry'] is not None and
                self.module.params['expiry'] != user_details.get('expiry')):
            return True
    ...
    def _resolve_role_change(self, user_name, user_id, role_name, role_state, auth_user_id):
        """Determine and apply role changes. Returns True if role was changed."""
        role_flag = self.is_user_part_of_role(user_name, user_id, role_name)
        LOG.debug("role_flag %s", role_flag)

        should_remove = role_flag and role_state == "absent-for-user"
        should_add = not role_flag and role_state == "present-for-user"
        if not (should_remove or should_add):
            return False
        if self.module.check_mode:
            return True
        if should_remove:
    ...
    def modify_existing_user(self, user_name, user_id, role_name, role_state,
        ...
        shell = self.module.params['shell']
        role_changed = self._resolve_role_change(
    ...
    def delete_existing_user(self, provider_type, auth_user_id, access_zone,
        ...
        if self.module.check_mode:
            return True
        should_get_roles = role_name or role_state is not None or \
            access_zone.lower() == "system"
        if should_get_roles:
            for role in self.get_roles_for_user(user_name, user_id):
    ...
    def perform_module_operation(self):
        ...
        if state == "present":
            ...
            if not user_details:
                self.create_new_user(user_name, user_id, password, access_zone,
                                    provider_type, enabled, primary_group,
                                    home_directory, full_name, email,
                                    role_state, role_name, auth_user_id)
                changed = True
